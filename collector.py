...
class DruidCollector(object):
    ...
    def collect(self):
            ...
            for metric in query_metrics:
                ...
                if daemon in self.histograms[metric]:
                    for datasource in self.histograms[metric][daemon]:
                        ...
                        buckets_without_sum = [
                            (k, v) for k, v in buckets.items() if k != 'sum']
        ...
        broker_metrics = self._get_broker_counters()
        ...
        for daemon, metrics in [('broker', broker_metrics),
